...
flags.DEFINE_string('train_logdir', './Pascal_voc_seg/train_logdir','Where the checkpoint and logs are stored.')                                  # None
...
flags.DEFINE_string('tf_initial_checkpoint', './Pascal_voc_seg/tf_initial_checkpoint/model.ckpt','The initial checkpoint in tensorflow format.')        #None
# './Pascal_voc_seg/tf_initial_checkpoint/model.ckpt'
# Set to False if one does not want to re-use the trained classifier weights.
flags.DEFINE_boolean('initialize_last_layer', False,'Initialize the last layer.')                                                  # TRUE实用预训练所有权重
flags.DEFINE_boolean('last_layers_contain_logits_only', True,'Only consider logits as last layers or not.')                       # 类变时候才为True
...
flags.DEFINE_boolean('fine_tune_batch_norm', False,'Fine tune the batch norm parameters or not.')                                   # TRUE
...
flags.DEFINE_multi_integer('atrous_rates', [12, 24, 36],'Atrous rates for atrous spatial pyramid pooling.')                         #None
flags.DEFINE_integer('output_stride', 8,'The ratio of input to output spatial resolution.')                                           #16
...
# Dataset settings.
flags.DEFINE_string('dataset', 'id_card','Name of the segmentation dataset.')
flags.DEFINE_string('train_split', 'train','Which split of the dataset to be used for training')
flags.DEFINE_string('dataset_dir', './Pascal_voc_seg/tfrecord', 'Where the dataset reside.')                                    # None
